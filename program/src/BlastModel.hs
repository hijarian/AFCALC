...
dzdu p u = nval' * eval'  * divident' / divisor''
  where
    nval'  = (phi_0 p) * mfunc / pi / (v_0 p)
    eval'  = exp ( 0 :+ ((1 - alpha p) * pi)) 
    divident' = t1m4t p * t1p4t p * t2m4t p * t2p4t p
    divisor'' = (t1m4 u * t4m4 u) ** (1 - alpha p)  * (t1p4 u * t4p4 u) ** (1 + alpha p)
