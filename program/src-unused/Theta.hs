...
-- WARNING: theta-functions raising their values very quickly when arg is raising. This depends on behaviour of cos and sin of complex functions, which are very rapidly increasing their values.
...
-- Parameter q, however, depends on the main parameter tau, so we will make
